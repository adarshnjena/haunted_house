...
gui.close()
let f3 = gui.addFolder('Haunted House');
let message = {
    task: 'find the hidden message',
    hint: '(looks like a red flame)'
}
gui.add(message, "task")
gui.add(message, "hint")
...
scene.fog = new THREE.Fog('#262837', 2, 15);
...
//text
(new THREE.FontLoader()).load('Who_asks_Satan_Regular.json', font => {
    const geometry = new THREE.TextBufferGeometry('Hounted House',
        {
            font: font,
            size: 0.59,
            height: 0.001,
...
// Directional light
const moonLight = new THREE.DirectionalLight('#ffffff', 0.095)
...
//door light
const doorLight = new THREE.PointLight('#ff7d46', 0.81, 7)
